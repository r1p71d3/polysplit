...
lint:  ## run static analysis with flake8
	python -m black --check polysplit setup.py
	python -m flake8 polysplit setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black polysplit/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./polysplit

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v polysplit

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v polysplit --cov=polysplit --cov-branch --cov-fail-under=75 --cov-report term-missing
